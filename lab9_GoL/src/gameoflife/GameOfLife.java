...
public class GameOfLife {
    ...
    public TETile[][] nextGeneration(TETile[][] tiles) {
        ...
        for (int x =0 ; x<width ; x++){
            for (int y = 0; y<height; y++){
                int cntLive = countLiveNeighbors(tiles, x, y);
                // case live
                if (Tileset.CELL.equals(tiles[x][y])){
                    switch (cntLive){
                        case 2,3 -> nextGen[x][y] = Tileset.CELL;
                        default -> nextGen[x][y] = Tileset.NOTHING;
                    }
                }else{
                    switch (cntLive){
                        case 3 -> nextGen[x][y] = Tileset.CELL;
                        default -> nextGen[x][y] = Tileset.NOTHING;
                    }
                }
            }
    ...
    public void saveBoard() {
        ...
        for (int y = height-1; y>=0; y--){
            for (int x =0 ; x<width ; x++){
                if(Tileset.CELL.equals(currentState[x][y])){
                    content.append(1);
                }else if(Tileset.NOTHING.equals(currentState[x][y])){
                    content.append(0);
                }
            }
            content.append("\n");
    ...
    public TETile[][] loadBoard(String filename) {
        ...
        for(int i = 1; i< lines.length; i++){
            for (int j=0; j<lines[i].length(); j++){
                loadTiles[h-i][j] = (lines[i].charAt(j) == '1') ? Tileset.CELL : Tileset.NOTHING;
            }
        }
