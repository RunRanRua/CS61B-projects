...
public class Model {
    ...
    public boolean maxTileExists() {
        ...
        for(int r = 0; r<len;r++){
            for(int c =0; c<len;c++){
                if ( board.tile(r,c) != null && board.tile(r,c).value() == MAX_PIECE){
                    return true;
                }
            }
        }
    ...
    public boolean atLeastOneMoveExists() {
        ...
        // case 2 : adjacent tile
        int len = board.size();
        for(int r = 0; r<len-1;r++){
            for(int c=0; c<len-1;c++){
                // check up + right
                if (board.tile(c,r).value() == board.tile(c,r+1).value() || board.tile(c,r).value() == board.tile(c+1,r).value() ){
                    return true;
                }
            }
            if( board.tile(len-1,r).value() == board.tile(len-1,r+1).value() ){
                return true;
            }
        }
        for (int c=0; c<len-1;c++){
            if ( board.tile(c,len-1).value() == board.tile(c+1,len-1).value() ){
                return true;
            }
        }
        return false;
